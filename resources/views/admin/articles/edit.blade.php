        ...
        <form action="{{ route('admin.articles.update', $article->id) }}" method="POST">
            ...
            <div class="row g-4">
                <div class="col-md-12">
                    ...
                    <input type="text" class="form-control custom-input @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $article->title) }}" required>
                    @error('title')
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control custom-input @error('category') is-invalid @enderror" id="category" name="category" value="{{ old('category', $article->category) }}" required>
                    @error('category')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        <input type="number" min="1" class="form-control custom-input @error('read_time') is-invalid @enderror" id="read_time" name="read_time" value="{{ old('read_time', $article->read_time) }}" required>
                ...
                <div class="col-md-12">
                    ...
                    <div class="d-flex gap-3 align-items-start">
                        <div class="flex-grow-1">
                            <input type="url" class="form-control custom-input @error('image_url') is-invalid @enderror" id="image_url" name="image_url" value="{{ old('image_url', $article->image_url) }}" required>
                            <small class="text-muted mt-1 d-block"><i class="ph ph-info"></i> Masukkan link URL gambar yang valid (berawalan http/https).</small>
                            @error('image_url')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="d-none d-md-block" style="width: 120px; height: 80px; border-radius: 12px; background-image: url('{{ $article->image_url }}'); background-size: cover; background-position: center; border: 1px solid var(--border-color);"></div>
                    </div>
                </div>

                <div class="col-md-12">
                    <label for="summary" class="form-label fw-semibold" style="color: var(--text-main);">Ringkasan (Summary)</label>
                    <textarea class="form-control custom-input @error('summary') is-invalid @enderror" id="summary" name="summary" rows="3" required>{{ old('summary', $article->summary) }}</textarea>
                    @error('summary')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <textarea class="form-control custom-input @error('content') is-invalid @enderror" id="content" name="content" rows="12" required>{{ old('content', $article->content) }}</textarea>
